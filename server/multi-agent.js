    ...
    try {
      ...
      switch (data.type) {
        case 'join':
          ...
          // 如果是 agent 连接
          // 兼容两种 join: data.agentId (新协议) 或 data.role (bot-bot 旧协议)
          const agentKey = data.agentId || (data.role && agentState[data.role] ? data.role : null);
          if (agentKey) {
            agentState[agentKey].online = true;
            agentState[agentKey].ws = ws;
            agentState[agentKey].lastSeen = Date.now();
            currentUser.role = agentKey;
          }
